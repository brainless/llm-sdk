...
/// OpenCode Zen provider client
///
/// Serves multiple free models (Big Pickle, DeepSeek, MiniMax, Nemotron, etc.)
/// via a single OpenAI-compatible Chat Completions endpoint.
...
impl OpenCodeZenClient {
    /// Create a new OpenCode Zen client (no API key required for free models)
    ...
    pub fn message_builder(&self) -> crate::glm::GlmMessageBuilder<'_, OpenCodeZenClient> {
        crate::glm::GlmMessageBuilder::new(self)
    }
}

impl Default for OpenCodeZenClient {
    fn default() -> Self {
        Self::new().expect("Failed to create default OpenCodeZenClient")
    }
}

#[async_trait]
impl crate::client::LlmClient for OpenCodeZenClient {
    async fn complete(
        &self,
    ...
    fn model_name(&self) -> &str {
        crate::models::zen::BIG_PICKLE_ID
    }
}
